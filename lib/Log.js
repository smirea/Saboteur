...
var Logger = function (name) {
  ...
  if (!this.name) {
    try {
      ...
      var name = stack[2].replace(/^(\s*at )new /, '$1');
      name = name.split(/\s+/);
      name = name[2].split('.');
  ...
  this.group = function group (name) {
    this._message(Color.colorize('l;gray', this.name + ' (' + name + '):'));
  ...
  this.warn = function warn () {
    var func_name = arguments.callee.caller.name;
    var args = this._to_array(arguments);
    args.unshift(this._make_name(func_name), Color.colorize('l;yellow', 'WARNING'));
    return this._message(args);
  };
  ...
  this.error = function error () {
    var func_name = arguments.callee.caller.name;
    var args = this._to_array(arguments);
    args.unshift(this._make_name(func_name), Color.colorize('l;red', 'ERROR'));
    return this._message(args);
  };
...
function B () {
  ...
  this.call_b_debug = function call_debug () {
    this._log.log('aloha');
    this._log.group('going once');
    this._log.log('aha');
    this._log.group('goung twice');
    this._log.log('lets debug');
    this._log.warn('and give a warning');
    this._log.error('ouch an error');
    this._log.group_end();
    this._log.log('more debugging');
    this._log.log('more debugging');
    this._log.group_end();
    this._log.log('and more logging');
  }
  this.call_b_debug();
}
