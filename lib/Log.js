...
var Logger = {
  ...
  stack_line_structure: {
    ...
    order: [null, null, 'name', null, 'as', 'file', 'line', 'column']
  },
  _groups: [],
  ...
  log: function (str) {
    var caller = Logger.get_caller();
    console.log('[%s:%d] %s', caller.name, caller.line, str);
  },
  ...
  log: function log () {
    ...
    args.unshift(this._make_name(caller));
  ...
  warn: function warn () {
    var caller = Logger.get_caller();
    var args = this._to_array(arguments);
    args.unshift(this._make_name(caller), Color.colorize('l;yellow', 'WARNING'));
    return this._message(args);
  },
  ...
  error: function error () {
    var caller = Logger.get_caller();
    var args = this._to_array(arguments);
    args.unshift(this._make_name(caller), Color.colorize('l;red', 'ERROR'));
    return this._message(args);
  },
