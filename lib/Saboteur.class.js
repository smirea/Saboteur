var Saboteur = Class.extend({
  ...
  doHeal : function(dryrun, playerID, cards, targetID, cardID) {
    var player = this.players[playerID];
    var target = this.players[targetID];
    var card = this.factory.get('game', cardID);
    if ( player && target && card
      && cards.length == this.opt.healDiscard
      && this.validateCards(playerID, cards)
  ...
  validateCards : function(playerID, givencards) {
    ...
    for (var i in givencards) {
      ...
      console.log(i, givencards, this.players[playerID].private.getCardPosition(givencards[i]));
