var Saboteur = Class.extend({
  init  : function( options, cards ){
    var com = this.com;
    U.extend( com.opt, SaboteurOptions.options, options );
    U.extend( true, com.cards, SaboteurOptions.cards, cards );
    U.extend( com.gameDeckList, com.cards.path, com.cards.action );
    U.extend( com.roleDeckList, com.cards.role );
    
    // create the card decks
    com.gameDeck = this.shuffle( this.makeDeck( com.gameDeckList ) );
    com.roleDeck = this.shuffle( this.makeDeck( com.roleDeckList ) );
    ...
    this.discardFromDeck( com.gameDeck, com.opt.discardFromDeck );
  },
  ...
  createPlayers : function( players ){
    var team = {};
    // TODO: unhack!!!! we need to somehow have a preprocessing step where get
    // the socketIDs together and use that as a pool for generating players
    for( var i in players ){
   ...
   */
  discardFromDeck : function( deck, number ){
    deck = deck || this;
  ...
  doDiscard : function(playerID, cards) {
    ...
    /*
    for (var st in player) {
      console.log(st);
      console.log(' *** ');
      console.log(player[st]);
    };
    */
    // check for all cards being right
    for (var i in cards) {
    ...
    for (var i in cards) {
      console.log('before');
      console.log(typeof player.cards[i]);

      player.cards[i] = this.drawCard(this.com.gameDeck);
