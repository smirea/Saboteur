Map = Class.extend({
  ...
  getCardAt  : function( x, y ){
    ...
    if( pos.x >= 0 && pos.x <= this.maxCards * 2 && pos.y >= 0 && pos.y <= this.maxCards * 2 ){
      ...
      if (cardInfo) {
        return this.factory.get.apply(this.factory, cardInfo);
      } else {
        return null;
      };
    } else {
  ...
  checkInsertCardAt : function(cardID, x, y) {
    return this._checkInsertCardAt([Deck.game, cardID], x, y);
  },
  ...
  insertCardAt : function( cardID, x, y) {
    return this._insertCardAt([ Deck.game, cardID ], x, y);
  },
  ...
  canReachGold : function( player ) {
    ...
    var newTile = function(dir) {
      // 0 is untouched, 1 is entered through here, 2 is finished checking
      var d = dir || {};
      var ret = {};
      ret[D.top] = d[D.top] || 0;
      ret[D.right] = d[D.right] || 0;
      ret[D.bottom] = d[D.bottom] || 0;
      ret[D.left] = d[D.left] || 0;
     return ret;
    };
    
    var checkAndAddTile = function(pos, card, dir) {
      var side = card.sides[dir];
      var neighbour = this.neighbour(pos.x, pos.y, dir);
      var canConnect = side.hasPath && this.isCardAt(neighbour.x, neighbour.y);
      if (canConnect) {
        var newDir = {};
        newDir[D.opposite(dir)] = 1;
        var tile = newTile(newDir);
        
        finalStack.push(neighbour);
        cards[neighbour.x][neighbour.y] = tile;
        return true;
      };
      return false;
    };
    ...
    for (var i in startStack) {
      ...
      var startTile = newTile();
      var card = this.getCardAt(pos.x, pos.y);
      for (var dir in card.sides) {
        checkAndAddTile(pos, card, dir);
        startTile[dir] = 2;
      };
      cards[pos.x][pos.y] = startTile;
    };
    
    while (finalStack) {
      var pos = finalStack.pop();
      var card = this.getCardAt(pos.x, pos.y);
      var tile = cards[pos.x][pos.y];
      var sides = card.sides;
      var repeat = true; // need to loop twice...
      while(repeat) {
        for (var dir in card.sides) {
          if (tile[dir] == 1) { // if we came in through here
            var side = card.sides[dir]
            var links = side.links
            for (var i in links) {
              if (tile[links[i]] == 0) { // if this can lead to a new place
                checkAndAddTile(pos, links[i]);
                tile[links[i]] = 1;
              };
            };
            tile[dir] == 2;
      ...
      //TODO: continue from here..check win condition; check proximity to goal
    };
    /*
    for (var i in startStack) {
      var pos = startStack[i];
