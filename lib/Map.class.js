...
Map = Class.extend({
  ...
  init        : (function(){
    ...
    function add_starting_cards(){
      for( var i in this.startCards ){
        this._insertCardAt( [ 'start', this.startCards[i][0] ], this.startCards[i][1], this.startCards[i][2] );
      };
      for( var i in this.goalCards ){
        this._insertCardAt( [ 'goal', this.goalCards[i][0] ], this.goalCards[i][1], this.goalCards[i][2] );
      };
    };
  })(),
  ...
  getCardAt  : function( x, y ){
    ...
    if( pos.x >= 0 && pos.x <= this.maxCards * 2 && pos.y >= 0 && pos.y <= this.maxCards * 2 ){
      return F.get.apply(null, this.cards[pos.x][pos.y]);
    } else {
  ...
  _checkInsertCardAt : function( cardWrapper, x, y ){
    var aPathCard = F.get(cardWrapper[0], cardWrapper[1]);
    var pos = this._realIndex( x, y );
    if( pos.x >= 0 && pos.y >= 0 ){
